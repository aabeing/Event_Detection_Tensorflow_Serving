...
def get_prediction(av_filename,image_path):
    ...
    out_sec = np.zeros(shape=(duration * 1000,scores.shape[1]))
    # out_sec = np.zeros(shape=(duration,scores.shape[1]))
    for i in range(0,out_sec.shape[0]):
        index = ((i)/out_sec.shape[0]) * scores.shape[0]
        index = int(index)
        # print(index,i)
        out_sec[i] = scores[index]
